...
// app.post('/agaps-round', (req: Request, res: Response) => {

//     const timestamp = Date.now();
//     const formattedTimestamp = new Date(timestamp).toISOString().replace('T', ' ').split('.')[0];

//     const { course, round, hole, par, score, fairway, green, approach, penalty, putts } = req.body;

//     const query = `INSERT INTO agapsRound (course, round, hole, par, score, fairway, green, approach, penalty, putts, formattedTimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

//     db.query(query, [course, round, hole, par, score, fairway, green, approach, penalty, putts, formattedTimestamp], (err: any, data: any) => {
//         if (err) {
//             return res.json(err);
//         } else {
//             console.log(data);
...
// // get agaps-round by id
// app.get('/agaps-round/:id', (req: Request, res: Response) => {
//     const { id } = req.params;
//     const query = `SELECT * FROM agapsRound WHERE id = ?`;
//     db.query(query, [id], (err: any, data: any) => {
//         if (err) {
//             return res.json(err);
//         } else {
